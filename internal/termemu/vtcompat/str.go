...
func (t *State) handleSTR() {
	...
	case ']': // OSC - operating system command
		...
		case 52:
			...
			payload := strings.TrimSpace(s.argString(2, ""))
			if payload == "?" {
				break
			}
			decoded, err := decodeOSC52(payload)
			if err != nil {
				t.logf("invalid osc52 payload: %v\n", err)
				break
			}
			if t.clipboardFn != nil {
				t.clipboardFn(decoded)
			}
		default:
...
func decodeOSC52(payload string) (string, error) {
	if payload == "" {
		return "", nil
	}
