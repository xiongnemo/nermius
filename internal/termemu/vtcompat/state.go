...
func (t *State) Cell(x, y int) Glyph {
	cell := t.lines[y][x]
	return t.applyColorOverride(cell)
}

// HistoryCell returns a glyph from the scrollback buffer.
func (t *State) HistoryCell(x, y int) Glyph {
	if y < 0 || y >= len(t.scrollback) {
		return t.blankGlyph()
	}
	if x < 0 || x >= len(t.scrollback[y]) {
		return t.blankGlyph()
	}
	cell := t.scrollback[y][x]
	return t.applyColorOverride(cell)
}

// ScrollbackRows returns the number of saved scrollback rows.
func (t *State) ScrollbackRows() int {
	return len(t.scrollback)
}

func (t *State) applyColorOverride(cell Glyph) Glyph {
...
func (t *State) CursorStyle() CursorStyle {
...
// SetClipboardHandler registers a callback for OSC 52 clipboard writes.
func (t *State) SetClipboardHandler(handler func(string)) {
	t.clipboardFn = handler
}
...
func (t *State) reset() {
	t.cur = t.defaultCursor()
	t.saveCursor()
	t.cursorStyle = CursorStyle{Shape: CursorShapeBlock, Blink: true}
	t.scrollback = nil
	for i := range t.tabs {
		t.tabs[i] = false
	}
...
func (t *State) resize(cols, rows int) bool {
	...
	if t.cols != 0 && cols != t.cols {
		t.scrollback = nil
	}
	slide := t.cur.Y - rows + 1
	if slide > 0 {
		t.appendScrollback(t.lines[:slide])
...
func (t *State) scrollUp(orig, n int) {
	n = clamp(n, 0, t.bottom-orig+1)
	if t.mode&ModeAltScreen == 0 && orig == 0 && t.bottom == t.rows-1 && n > 0 {
		t.appendScrollback(t.lines[orig : orig+n])
	}
...
func (t *State) appendScrollback(lines []line) {
	if t.scrollLimit <= 0 || len(lines) == 0 {
		return
	}
	for _, src := range lines {
		copied := make(line, len(src))
		copy(copied, src)
		t.scrollback = append(t.scrollback, copied)
	}
	if overflow := len(t.scrollback) - t.scrollLimit; overflow > 0 {
		t.scrollback = append([]line(nil), t.scrollback[overflow:]...)
	}
}
...
func (t *State) setCursorStyle(arg int) {
	switch arg {
	case 0, 1:
		t.cursorStyle = CursorStyle{Shape: CursorShapeBlock, Blink: true}
	case 2:
		t.cursorStyle = CursorStyle{Shape: CursorShapeBlock, Blink: false}
	case 3:
		t.cursorStyle = CursorStyle{Shape: CursorShapeUnderline, Blink: true}
	case 4:
		t.cursorStyle = CursorStyle{Shape: CursorShapeUnderline, Blink: false}
	case 5:
		t.cursorStyle = CursorStyle{Shape: CursorShapeBar, Blink: true}
	case 6:
		t.cursorStyle = CursorStyle{Shape: CursorShapeBar, Blink: false}
	default:
		t.logf("unknown cursor style %d\n", arg)
	}
}
